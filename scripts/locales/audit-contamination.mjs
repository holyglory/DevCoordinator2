...
for(const entry of entries){const matches=[]; for(const file of Object.values(entry.files).flat()){const ns=path.basename(file,'.json'); const data=JSON.parse(await fs.readFile(path.join(localeRoot,file),'utf8')); for(const [id,msg] of Object.entries(data)){const vals=typeof msg==='string'?[msg]:Object.values(msg?.forms||{}); for(const value of vals){if(typeof value!=='string')continue; const low=value.toLowerCase(); const words=(value.match(tokenRe)||[]).map(x=>x.toLowerCase()).filter(x=>stopWords.includes(x)); const phrase=phrasePatterns.some(p=>p.test(value)); if(phrase||new Set(words).size>=3)matches.push({namespace:ns,id,value,reason:phrase?'phrase':'english-token-cluster'});}}} console.log(JSON.stringify({locale:entry.tag,status:entry.status,candidates:matches.length,matches},null,2));}
