  ...
  function create({ api, esc, compactNumber, durationMs, coverageText, identity }) {
    ...
    async function render(root, repositoryId, signal) {
      ...
      function showUsage(usage, append = false) {
        ...
        if (!append) {
          showCoverage(usage);
          window.DevCoordinatorI18n.bind(find('scope-total'), () => window.DevCoordinatorI18n.t("performance.value1_total_value2_value3_0a3988", {value1: reference ? 'Review' : 'Period', value2: amount(total), value3: reference ? '' : ' · ' + period(start, end)}));
