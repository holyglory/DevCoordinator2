...
const state = {
  who: null,
  healthRange: '24h',
  healthIncidentsOpen: false,
  healthShowDismissed: false,
  healthIncidentGroups: [],
  healthDismissedIncidents: new Set(),
  healthDismissedInitialized: false,
  usageRange: '1h',
  codexUsageRange: '24h',
  progressPeriod: 'day',
...
const HEALTH_DEVELOPMENT_ALERT_KINDS = new Set(['test_scratch']);

function loadHealthDismissedIncidents() {
  if (state.healthDismissedInitialized) return;
  state.healthDismissedInitialized = true;
  try {
    const saved = JSON.parse(localStorage.getItem('dc2-health-dismissed-incidents') || '[]');
    if (Array.isArray(saved)) state.healthDismissedIncidents = new Set(saved.filter((value) => typeof value === 'string'));
  } catch { state.healthDismissedIncidents = new Set(); }
}

function saveHealthDismissedIncidents() {
  try { localStorage.setItem('dc2-health-dismissed-incidents', JSON.stringify([...state.healthDismissedIncidents])); } catch {}
}

function healthIncidentDeployment(alert, deployments) {
  const subject = String(alert?.subject_id || '');
  const deploymentId = alert?.subject_kind === 'deployment' ? subject : subject.split('/')[0];
  return deployments.find((deployment) => deployment.deployment_id === deploymentId) || null;
}

function healthAlertIsReportable(alert, deployments) {
  if (!alert || HEALTH_DEVELOPMENT_ALERT_KINDS.has(alert.kind)) return false;
  // An incident exists in the operator queue only when the health service has
  // emitted an explicit alert. Test/worktree subjects are development signals;
  // an agent that handles a condition without reporting it therefore produces
  // no operator incident here.
  if (alert.subject_kind === 'worktree' || alert.subject_kind === 'test') return false;
  const deployment = healthIncidentDeployment(alert, deployments);
  return !deployment || deployment.source !== 'worktree';
}

function healthIncidentGroups(summary, repos) {
  const unhealthyDeployments = summary?.unhealthy_deployments || [];
  const knownDeployments = (repos?.repositories || []).flatMap((row) => row.deployments || []);
  const unhealthyById = new Map(unhealthyDeployments.map((deployment) => [deployment.deployment_id, deployment]));
  const repositoryRows = new Map((repos?.repositories || []).map((row) => [row.repository_id, row]));
  const groups = new Map();
  for (const alert of summary?.alerts || []) {
    if (!healthAlertIsReportable(alert, knownDeployments)) continue;
    const deploymentRow = healthIncidentDeployment(alert, knownDeployments);
    const deployment = deploymentRow ? { ...deploymentRow, ...(unhealthyById.get(deploymentRow.deployment_id) || {}) } : null;
    const key = deployment ? `deployment:${deployment.deployment_id}` : `alert:${alert.alert_key}`;
    let group = groups.get(key);
    if (!group) {
      group = {
        id: key,
        deployment,
        repository: deployment ? repositoryRows.get(deployment.repository_id) : null,
        alerts: [],
    ...
    const subject = String(alert.subject_id || '');
    const component = deployment && subject.startsWith(`${deployment.deployment_id}/`)
      ? subject.slice(deployment.deployment_id.length + 1) : null;
    group.alerts.push({ ...alert, component });
    if (alert.severity === 'critical') group.severity = 'critical';
    if (Date.parse(alert.opened_at) < Date.parse(group.opened_at)) group.opened_at = alert.opened_at;
    if (Date.parse(alert.last_seen_at || alert.opened_at) > Date.parse(group.last_seen_at)) group.last_seen_at = alert.last_seen_at || alert.opened_at;
  }
  return [...groups.values()].sort((left, right) => Date.parse(right.last_seen_at) - Date.parse(left.last_seen_at));
}

function healthIncidentHeadline(group) {
  if (!group.deployment) return group.alerts[0]?.message || 'Host condition needs attention';
  const components = [...new Set(group.alerts.map((alert) => alert.component).filter(Boolean))];
  if (components.length === 1) return `${components[0]} needs attention`;
  if (components.length > 1) return `${components.length} components need attention`;
  return `${group.deployment.name} needs attention`;
}

function healthIncidentDetails(group) {
  if (!group.deployment) return '';
  const reasons = group.deployment.reasons || [];
  const alertComponents = new Set(group.alerts.map((alert) => alert.component).filter(Boolean));
  const visibleReasons = reasons.filter((reason) => !alertComponents.size || alertComponents.has(reason.component));
  if (!visibleReasons.length) return '';
  return `<ul class="health-incident-reasons">${visibleReasons.map((reason) => `<li><span class="mono">${esc(reason.component)}</span> ${badge(reason.state, 'bad')}${reason.detail ? ` <span class="muted">${esc(reason.detail)}</span>` : ''}</li>`).join('')}</ul>`;
}

function healthIncidentCard(group, dismissed = false) {
  const deployment = group.deployment;
  const repositoryName = group.repository?.display_name || deployment?.repository_name || 'Host';
  const repositoryPath = group.repository?.root_path;
  const signalCount = group.alerts.length;
  const action = dismissed
    ? `<button class="btn btn-small" type="button" data-health-restore="${esc(group.id)}"><span data-i18n="health.restore_a76e13">Restore</span></button>`
    : `<button class="btn btn-small" type="button" data-health-dismiss="${esc(group.id)}"><span data-i18n="health.dismiss_48845b">Dismiss</span></button>`;
  return `<article class="health-incident-card ${group.severity === 'critical' ? 'is-critical' : ''}" data-health-incident-id="${esc(group.id)}">
    <div class="health-incident-marker" aria-hidden="true"></div>
    <div class="health-incident-time"><strong>${window.DevCoordinatorI18n.computedMarkup(() => ago(group.last_seen_at))}</strong><span>${window.DevCoordinatorI18n.computedMarkup(() => ago(group.opened_at))} first seen</span></div>
    <div class="health-incident-content">
      <div class="cardhead"><strong>${esc(repositoryName)}</strong>${deployment ? `<span class="muted">${esc(deployment.name)}@${esc(deployment.source)}</span> ${badge(deployment.state)}` : badge('host', '')} ${badge(group.severity, group.severity === 'critical' ? 'bad' : 'warn')}</div>
      ${repositoryPath ? `<div class="muted mono health-incident-root">${esc(repositoryPath)}</div>` : ''}
      <p class="health-incident-headline">${esc(healthIncidentHeadline(group))}</p>
      <div class="health-incident-meta"><span>${window.DevCoordinatorI18n.markup("health.reportedSignals", { count: signalCount })}</span><span>last seen ${window.DevCoordinatorI18n.computedMarkup(() => ago(group.last_seen_at))}</span></div>
      ${healthIncidentDetails(group)}
      <div class="actions">${deployment ? `${lifecycleButtons(deployment.deployment_id, null, 'btn btn-small', deployment.state)}<a class="btn btn-small" href="#/deployments/${esc(deployment.deployment_id)}"><span data-i18n="health.details_logs_a2ea65">Details &amp; logs</span></a>` : ''}${action}</div>
    </div>
  </article>`;
}

function reportedIncidentsSection() {
  const groups = state.healthIncidentGroups || [];
  const active = groups.filter((group) => !state.healthDismissedIncidents.has(group.id));
  const dismissed = groups.filter((group) => state.healthDismissedIncidents.has(group.id));
  const showingDismissed = state.healthShowDismissed;
  const visible = showingDismissed ? dismissed : active;
  const expanded = state.healthIncidentsOpen || showingDismissed;
  const countLabel = active.length ? `${active.length} need attention` : 'Nothing reported';
  const body = visible.length
    ? `<div class="health-incident-timeline">${visible.map((group) => healthIncidentCard(group, showingDismissed)).join('')}</div>`
    : `<div class="health-clear-state">${(showingDismissed ? window.DevCoordinatorI18n.markup("health.no_dismissed_incidents_375c5b") : window.DevCoordinatorI18n.markup("health.no_reported_incidents_need_attention_ea3a12"))}</div>`;
  return `<section class="health-section health-reported-incidents" aria-labelledby="health-incidents-title">
    <div class="health-incident-disclosure ${active.length ? 'has-incidents' : ''}">
      <button class="health-incident-toggle" type="button" data-health-incidents-toggle aria-expanded="${expanded}">
        <span class="health-incident-alert-dot" aria-hidden="true"></span><span class="health-incident-toggle-copy"><strong id="health-incidents-title"><span data-i18n="health.reported_incidents_5c3cb2">Reported incidents</span></strong><span class="muted">${esc(countLabel)}</span></span><span class="health-incident-count">${active.length}</span><span class="health-incident-toggle-label">${(expanded ? window.DevCoordinatorI18n.markup("health.collapse_be6eb1") : window.DevCoordinatorI18n.markup("health.expand_07548c"))}</span>
      </button>
      <div class="health-incident-disclosure-actions"><button class="btn btn-small" type="button" data-health-show-dismissed ${dismissed.length ? '' : 'disabled'} aria-pressed="${showingDismissed}">View dismissed${dismissed.length ? ` (${dismissed.length})` : ''}</button></div>
    </div>
    ${expanded ? `<div class="health-incident-body" aria-live="polite"><div class="health-incident-body-heading"><span>${(showingDismissed ? window.DevCoordinatorI18n.markup("health.dismissed_incidents_c4dec9") : window.DevCoordinatorI18n.markup("health.timeline_9dcff9"))}</span>${showingDismissed ? "<span class=\"muted\"><span data-i18n=\"health.restore_an_incident_to_return_it_to_the_queue_6199d2\">Restore an incident to return it to the queue.</span></span>" : "<span class=\"muted\"><span data-i18n=\"health.only_explicitly_reported_operator_incidents_appe_cc467a\">Only explicitly reported operator incidents appear here.</span></span>"}</div>${body}</div>` : ''}
  </section>`;
}

function bindHealthIncidentControls(root = main) {
  root.querySelectorAll('[data-health-incidents-toggle]').forEach((button) => {
    button.addEventListener('click', () => { state.healthIncidentsOpen = !state.healthIncidentsOpen; refreshHealthIncidentSection(); });
  });
  root.querySelectorAll('[data-health-show-dismissed]').forEach((button) => {
    button.addEventListener('click', () => { if (button.disabled) return; state.healthShowDismissed = !state.healthShowDismissed; state.healthIncidentsOpen = true; refreshHealthIncidentSection(); });
  });
  root.querySelectorAll('[data-health-dismiss], [data-health-restore]').forEach((button) => {
    button.addEventListener('click', () => {
      const key = button.dataset.healthDismiss || button.dataset.healthRestore;
      if (button.dataset.healthDismiss) state.healthDismissedIncidents.add(key); else state.healthDismissedIncidents.delete(key);
      saveHealthDismissedIncidents();
      state.healthShowDismissed = false;
      toast(() => (button.dataset.healthDismiss ? window.DevCoordinatorI18n.t("common.incident_dismissed_d1a6dd") : window.DevCoordinatorI18n.t("common.incident_restored_783c83")), 'ok');
      refreshHealthIncidentSection();
    });
  });
}

function refreshHealthIncidentSection() {
  const current = $('.health-reported-incidents');
  if (!current) return;
  current.outerHTML = reportedIncidentsSection();
  const next = $('.health-reported-incidents');
  bindHealthIncidentControls(next);
  bind(next);
}

function healthDeploymentSummary(deployments, repositoryId) {
  const counts = new Map();
  for (const deployment of deployments || []) counts.set(deployment.state, (counts.get(deployment.state) || 0) + 1);
  const summary = [...counts.entries()].map(([stateName, count]) => `<span class="health-deployment-count">${badge(stateName)} <strong>${count}</strong></span>`).join('') || "<span class=\"muted\"><span data-i18n=\"health.no_deployments_9790ef\">No deployments</span></span>";
  const details = (deployments || []).map((deployment) => `<span><a href="#/deployments/${esc(deployment.deployment_id)}">${esc(deployment.name)}@${esc(deployment.source)}</a> ${badge(deployment.state)}</span>`).join('');
  return `<div class="health-deployment-summary">${summary}</div>${deployments?.length ? `<button class="health-workers-toggle" type="button" data-health-workers-toggle="${esc(repositoryId)}" aria-expanded="false">${window.DevCoordinatorI18n.markup("health.show_deployments_value2_7bd0ee", {value2: deployments.length})}</button><div class="health-workers-list" data-health-workers="${esc(repositoryId)}" hidden>${details}</div>` : ''}`;
}
function bindHealthRepositoryControls(root = main) {
  root.querySelectorAll('[data-health-workers-toggle]').forEach((button) => {
    button.addEventListener('click', () => {
      const detail = button.parentElement?.querySelector(`[data-health-workers="${CSS.escape(button.dataset.healthWorkersToggle)}"]`);
      if (!detail) return;
      const expanded = detail.hidden;
      detail.hidden = !expanded;
      button.setAttribute('aria-expanded', String(expanded));
      window.DevCoordinatorI18n.bind(button, () => (expanded ? window.DevCoordinatorI18n.t("common.hide_deployments_789b75") : window.DevCoordinatorI18n.t("common.show_deployments_value1_0f82d6", {value1: detail.children.length})));
    });
  });
}
function reconciliationSection(host) {
  const reconciliation = host.reconciliation || {};
  return `<section class="health-section" aria-labelledby="health-reconciliation-title"><h2 id="health-reconciliation-title"><span data-i18n="health.reconciliation_c24d50">Reconciliation</span></h2><dl class="health-reconciliation">
    <div><dt><span data-i18n="health.cpu_db9a4c">CPU</span></dt><dd><span><span data-i18n="health.managed_8f2de6">Managed</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => pct(reconciliation.managed_cpu_percent))}</strong></span><span><span data-i18n="health.devcoordinator_b19f38">DevCoordinator</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => pct(reconciliation.daemon_cpu_percent))}</strong></span><span><span data-i18n="health.shared_unattributed_aa4d5b">Shared / unattributed</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => pct(reconciliation.other_cpu_percent))}</strong></span><span class="health-reconciliation-total"><span data-i18n="health.host_4a8231">Host</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => pct(host.cpu_percent))}</strong></span></dd></div>
    <div><dt><span data-i18n="health.memory_c3963a">Memory</span></dt><dd><span><span data-i18n="health.managed_8f2de6">Managed</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => bytes(reconciliation.managed_memory))}</strong></span><span><span data-i18n="health.devcoordinator_b19f38">DevCoordinator</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => bytes(reconciliation.daemon_memory))}</strong></span><span><span data-i18n="health.shared_unattributed_aa4d5b">Shared / unattributed</span> <strong>${window.DevCoordinatorI18n.computedMarkup(() => bytes(reconciliation.other_memory))}</strong></span></dd></div>
  </dl></section>`;
}
const viewHealth = guard(async (sub) => {
  if (sub === 'containers') return viewContainers();
  loadHealthDismissedIncidents();
  ...
  state.healthIncidentGroups = summary ? healthIncidentGroups(summary, repos) : [];
  ...
  const activeIncidentCount = state.healthIncidentGroups.filter((group) => !state.healthDismissedIncidents.has(group.id)).length;
  const criticalIncidentCount = state.healthIncidentGroups.filter((group) => !state.healthDismissedIncidents.has(group.id) && group.severity === 'critical').length;
  const tiles = summary ? `<div class="health-summary" data-ui-region="health-primary">
    <section class="health-panel health-capacity-panel" aria-labelledby="health-capacity-title"><div class="health-panel-heading"><h2 id="health-capacity-title"><span data-i18n="health.host_capacity_c36874">Host capacity</span></h2><span class="muted"><span data-i18n="health.now_ed5eb9">now</span></span></div><div class="health-capacity-grid">
      <div class="health-capacity-card"><div class="k"><span data-i18n="health.cpu_db9a4c">CPU</span> <span>${window.DevCoordinatorI18n.markup("health.value1_cores_cfde1b", {value1: h.ncpu ?? '?'})}</span></div><div class="health-capacity-value ${h.cpu_percent > 90 ? 'bad' : ''}"><strong>${window.DevCoordinatorI18n.computedMarkup(() => pct(h.cpu_percent))}</strong></div>${meter((h.cpu_percent ?? 0) / 100)}</div>
      <div class="health-capacity-card"><div class="k"><span data-i18n="health.memory_c3963a">Memory</span></div><div class="health-capacity-value"><strong>${window.DevCoordinatorI18n.computedMarkup(() => bytes(h.memory_used))}</strong><span>of ${window.DevCoordinatorI18n.computedMarkup(() => bytes(h.memory_total))}</span></div>${meter(memFrac)}</div>
      <div class="health-capacity-card"><div class="k"><span data-i18n="health.storage_a69c4d">Storage</span></div><div class="health-capacity-value ${fsFrac > 0.9 ? 'bad' : ''}"><strong>${window.DevCoordinatorI18n.computedMarkup(() => bytes(h.fs_used))}</strong><span>of ${window.DevCoordinatorI18n.computedMarkup(() => bytes(h.fs_size))}</span></div>${meter(fsFrac)}</div>
      <div class="health-capacity-card"><div class="k"><span data-i18n="health.system_load_f58135">System load</span></div><div class="health-capacity-value health-load-value"><strong>${h.load_1 ?? '—'} / ${h.load_5 ?? '—'} / ${h.load_15 ?? '—'}</strong></div><div class="health-capacity-detail">1 / 5 / 15 min · Swap ${window.DevCoordinatorI18n.computedMarkup(() => bytes(h.swap_used))}</div></div>
    </div></section>
    <section class="health-panel health-status-panel health-history-panel" aria-labelledby="health-history-title"><div class="health-panel-heading"><h2 id="health-history-title"><span data-i18n="health.system_trends_eeef72">System trends</span></h2>${seg(['24h', '7d', '30d'], state.healthRange, 'health-range')}</div><div id="host-history" class="chartrow">${skeleton(3)}</div></section>
  </div>` : stateBlock('denied', () => window.DevCoordinatorI18n.t("common.value1_server_wide_health_is_administrator_only_e001e0", {value1: denied}));
  const rows = repos.repositories.map((r) => `<tr><td class="wrap health-repository-name" data-label="Repository" data-i18n-attrs='{"data-label":"health.repository_13d6ff"}'><strong>${esc(r.display_name)}</strong><div class="muted mono">${(r.root_path ? esc(r.root_path) : window.DevCoordinatorI18n.markup("health.repository_root_unavailable_1d7ecd"))}</div></td><td data-label="CPU" data-i18n-attrs='{"data-label":"health.cpu_db9a4c"}'><span class="health-metric-with-trend">${window.DevCoordinatorI18n.computedMarkup(() => pct(r.cpu_percent))} ${spark(r.trend_cpu)}</span></td><td data-label="Memory" data-i18n-attrs='{"data-label":"health.memory_c3963a"}'><span class="health-metric-with-trend">${window.DevCoordinatorI18n.computedMarkup(() => bytes(r.memory_bytes))} ${spark(r.trend_memory)}</span></td><td data-label="Storage" data-i18n-attrs='{"data-label":"health.storage_a69c4d"}'><span class="health-metric-with-trend">${window.DevCoordinatorI18n.computedMarkup(() => bytes(r.storage_bytes))} ${spark(r.trend_storage)}</span></td><td data-label="Health" data-i18n-attrs='{"data-label":"health.health_558984"}'>${badge(r.health)}</td><td class="wrap health-deployments" data-label="Deployments" data-i18n-attrs='{"data-label":"health.deployments_842a46"}'>${healthDeploymentSummary(r.deployments, r.repository_id)}</td></tr>`).join('');
  main.innerHTML = `<div class="health-page-heading">${pageHeading('Health', '#/health')}<div class="health-heading-actions"><span class="health-heading-status ${activeIncidentCount ? 'has-incidents' : ''}"><span class="health-incident-alert-dot" aria-hidden="true"></span><strong>${(activeIncidentCount ? `${activeIncidentCount} reported incidents` : window.DevCoordinatorI18n.markup("health.no_reported_incidents_6b64fd"))}</strong>${criticalIncidentCount ? `<span class="muted">${window.DevCoordinatorI18n.markup("health.value1_critical_a27eba", {value1: criticalIncidentCount})}</span>` : ''}</span><a class="btn btn-small" href="#/health/containers"><span data-i18n="health.view_containers_e26e60">View containers</span></a></div></div>${summary ? reportedIncidentsSection() : ''}${tiles}${summary ? reconciliationSection(h) : ''}<section class="health-section health-repositories" aria-labelledby="health-repositories-title"><div class="health-section-heading"><h2 id="health-repositories-title"><span data-i18n="health.repositories_1e32af">Repositories</span></h2><span class="muted">${window.DevCoordinatorI18n.markup("health.value8_repositories_value9_healthy_944e0f", {value8: repos.repositories.length, value9: repos.repositories.filter((row) => row.health === 'healthy').length})}</span></div>${rows ? `<div class="tablewrap"><table class="health-repository-table"><thead><tr><th><span data-i18n="health.repository_13d6ff">Repository</span></th><th><span data-i18n="health.cpu_db9a4c">CPU</span></th><th><span data-i18n="health.memory_c3963a">Memory</span></th><th><span data-i18n="health.storage_a69c4d">Storage</span></th><th><span data-i18n="health.health_558984">Health</span></th><th><span data-i18n="health.deployments_842a46">Deployments</span></th></tr></thead><tbody>${rows}
    ${repos.devcoordinator ? `<tr class="health-attribution-row"><td data-label="Repository" data-i18n-attrs='{"data-label":"health.repository_13d6ff"}'><em><span data-i18n="health.devcoordinator_b19f38">DevCoordinator</span></em></td><td data-label="CPU" data-i18n-attrs='{"data-label":"health.cpu_db9a4c"}'>${window.DevCoordinatorI18n.computedMarkup(() => pct(repos.devcoordinator.cpu_percent))}</td><td data-label="Memory" data-i18n-attrs='{"data-label":"health.memory_c3963a"}'>${window.DevCoordinatorI18n.computedMarkup(() => bytes(repos.devcoordinator.memory_bytes))}</td><td data-label="Storage" data-i18n-attrs='{"data-label":"health.storage_a69c4d"}'>${window.DevCoordinatorI18n.computedMarkup(() => bytes(repos.devcoordinator.storage_bytes))}</td><td data-label="Health" data-i18n-attrs='{"data-label":"health.health_558984"}'></td><td data-label="Deployments" data-i18n-attrs='{"data-label":"health.deployments_842a46"}'></td></tr><tr class="health-attribution-row"><td data-label="Repository" data-i18n-attrs='{"data-label":"health.repository_13d6ff"}'><em><span data-i18n="health.shared_unattributed_aa4d5b">Shared / unattributed</span></em></td><td data-label="CPU" data-i18n-attrs='{"data-label":"health.cpu_db9a4c"}'>${window.DevCoordinatorI18n.computedMarkup(() => pct(repos.shared_unattributed.cpu_percent))}</td><td data-label="Memory" data-i18n-attrs='{"data-label":"health.memory_c3963a"}'>${window.DevCoordinatorI18n.computedMarkup(() => bytes(repos.shared_unattributed.memory_bytes))}</td><td class="wrap health-storage-cell" data-label="Storage" data-i18n-attrs='{"data-label":"health.storage_a69c4d"}'>${healthStorageBreakdown(repos.shared_unattributed.storage)}</td><td data-label="Health" data-i18n-attrs='{"data-label":"health.health_558984"}'></td><td data-label="Deployments" data-i18n-attrs='{"data-label":"health.deployments_842a46"}'></td></tr>` : ''}</tbody></table></div>` : stateBlock('empty', () => window.DevCoordinatorI18n.t("common.no_repositories_visible_to_you_40cf12"))}</section>`;
  bind(main);
  bindHealthIncidentControls(main);
  bindHealthRepositoryControls(main);
