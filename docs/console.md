...
| Progress period (Hour/Day/Week) | `progress.repository {repository_id, period}` | completed bars above the baseline and newly added work below it share one scale within each lane, so equal values have equal lengths; completion running totals, test/token evidence, forecast quality, Plan-ordered work, comparison totals, and exact values re-render from one bounded report |
