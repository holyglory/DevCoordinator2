...
| The verified release is active with a recoverable rollback point | PASS | clean `main` exactly matching `origin/main`; integrity-checked root-only backup `/var/backups/devcoordinator2/20260902T135644Z-pre-progressive-logs`; both services active and enabled with no restart warnings; authority database quick-check `ok`, live schema 14, retention 86,400 seconds / three histories, and first maintenance pass without error; live sign-in control enabled with no browser-console or page errors |

## Visual journey evidence review addendum (2026-09-02)

| Item | Result | Evidence |
|---|---|---|
| Formal UI checks publish one path-free ordered journey bundle inside their private retained run leaf; screenshot size, dimensions, SHA-256, action kinds, and finding kinds remain bound while selectors and values stay out | PASS | Rust executor environment/retention test; formal verifier complete self-test including its governed-run case; UI-audit importer/self-test; frozen-candidate 29/29 six-skill validation graph in `.devcoordinator/agent-validation/skills-20260902T222228Z-3120387-045c29/check-report.json` |
| Only administrators and trusted local callers can list or read exact evidence; the edge receives bounded verified chunks and never gains repository filesystem access | PASS | `tests/test_test_evidence.py`, access/operation/client/protocol tests, tamper/expiry/path-free and response-ceiling cases, security assumptions, edge 3/3 regression pass |
| A marked screenshot suggestion atomically creates ordinary Plan feedback, and replies, author edits, resolve/reopen, explicit deletion, reload, and exact Plan continuation work through the rendered interface | PASS | 283/283 focused interaction checks in `/tmp/dc2-visual-evidence-design-final/report.json`; database schema-15 migration and feedback lifecycle tests |
| The owner-selected third Product Design direction remains screenshot-first across desktop, tablet, and mobile without clipping, overlap, unreadable text, or document overflow | PASS | `design-qa.md` final result passed; complete Console matrix 1,625/1,625 in `/tmp/dc2-visual-evidence-console-release/report.json`; formal run `formal-web-ui-mtkn9iwc` checked 8/8 cells at 390, 559/560/561, 1179/1180/1181, and 1440 px with zero critical findings; all 16 images reviewed and eight passes finalized in `/tmp/formal-web-ui-verification-jdsp9G/manual-review.json` |
| Current source, execution plane, skills, and Console regressions are green | PASS | 347 Python tests passed with 26 environment skips; 103 Rust tests passed; Ruff clean; JavaScript syntax clean; edge 3/3; formal and UI-audit self-tests pass; complete skill graph 29/29 |

## Reported deployment and test-evidence defects addendum (2026-09-03)

| Item | Result | Evidence |
|---|---|---|
| Exact active and terminal log references remain readable when another retained run ended before sealing its metadata | PASS | focused malformed-neighbor, selected-run tamper, partial-tail, retention, restart, active, cancelled, timed-out, interrupted, superseded, passed, and failed cases; the reported consumer run and a new active-to-cancelled run both resolved through the installed catalogue |
| A routed Compose deployment cannot be reported ready until its allocated host port is published and reachable; an already-published unreachable route is withdrawn | PASS | unit port-binding/readiness checks; real first-apply and already-bad-upgrade Compose scenarios; exact installed deployment status and re-apply showed degraded/unrouted state instead of false running/public success |
| Full Compose start/apply restores running intent and failed candidates restore prior intent without losing finite-service receipts | PASS | real stop/start, successful apply, failed re-apply, finite setup, independent service, reload, and daemon-restart scenarios; installed service status reports running intent truthfully |
| Direct checks retain bounded evidence directory trees under exact run/source/config/proof identity and authorized callers can verify or materialize them without private paths | PASS | two-profile cross-language executor/daemon fixture; file/tree hash and chunk/materialization checks; empty, overlap, size, link, special-file, path, authorization, expiry, and tamper rejections |
| The frozen candidate passed all release gates before source-owned activation | PASS | 109 Rust tests; 361 ordinary Python tests; 28 isolated real-system integrations; 3 edge tests; strict Rust/Python linting; complete 29/29 skill and policy graph |

## Readable test-log viewer addendum (2026-09-03)

| Item | Result | Evidence |
|---|---|---|
| Opening Logs shows the newest bounded text without asking for line or byte numbers; human-readable stream names, collapsed details, older-output loading, latest refresh, literal search, likely-failure context, and stream switching all work | PASS | 299/299 focused browser interactions in `/tmp/dc2-readable-logs-focused-final/report.json`; long, short, active, failed, empty, expired/retry, paged-catalogue, and multi-stream fixtures; no Console call to `test.log.range` |
| Progressive pages preserve the text being read, do not repeat excerpts, and retain stable coordinates plus explicit untrusted-output labelling | PASS | cursor-specific browser assertions and the unchanged Rust tail/search paging contract; 109/109 Rust workspace tests |
| The text-first dialog remains contained and readable across mobile, the exact responsive boundary, and the owner-reported viewport | PASS | `design-qa.md` final result passed; formal run `formal-web-ui-mtlvv4t9` checked 5/5 cells at 390, 719/720/721, and 1095 px with zero critical findings or warnings; all ten images reviewed and five passes finalized in `/tmp/formal-web-ui-verification-nly2pg/manual-review.json` |
| The frozen implementation preserves the complete Console and repository contracts | PASS | complete Console matrix 1,641/1,641 in `/tmp/dc2-readable-logs-console-final/report.json`; 361 Python tests passed with 28 environment skips; Ruff and Rust formatting clean; edge 3/3; release executor built; complete skill/policy graph 29/29 |

## Collapsible deployment dashboard addendum (2026-09-03)

| Item | Result | Evidence |
|---|---|---|
| Each repository and deployment starts expanded, collapses independently by pointer or keyboard, retains identity and condition while closed, leaves siblings unchanged, and survives a same-session rerender | PASS | 315/315 focused interactions in `/tmp/dc2-collapse-dashboard-focused-2/report.json`; repository/record visibility, `aria-expanded`/`aria-controls`, Enter, Space, focus, lifecycle-action hiding, and state restoration assertions |
| Tests and Health show useful factual detail without crossing repository or permission boundaries | PASS | selected-run proof type, recency, tier, elapsed time, and output plus repository CPU, memory, storage, and deployment count asserted across deliberately divergent populated, missing, and restricted fixtures |
| Expanded and collapsed states remain readable at mobile, the exact responsive transition, and the owner-reported viewport | PASS | `design-qa.md` final result passed; formal run `formal-web-ui-mtm0yz1n` checked 15/15 states at 390, 619/620/621, and 1095 px with zero critical findings; ten intentional compact-region warnings reviewed; all thirty images finalized as fifteen passes in `/tmp/formal-web-ui-verification-qDKHVa/manual-review.json` |
| The frozen implementation preserves every Console and repository contract | PASS | complete Console matrix 1,659/1,659 in `/tmp/dc2-collapse-dashboard-console-release/report.json`; 109 Rust tests passed in the recorded serial mode required by the separate parallel-lease race; 361 Python tests passed with 28 environment skips; Ruff, JavaScript syntax, diff, public-artifact guard, and edge 3/3 passed |

## Live Tests and multi-bundle visual evidence addendum (2026-09-03)

| Item | Result | Evidence |
|---|---|---|
| Active Tests rows refresh to their terminal result without navigation, preserve an open dialog and focus, recover after a failed read, remove stale stop controls, and stop refreshing after completion or route departure | PASS | complete merged Console matrix 1,663/1,663 in `/tmp/dc2-test-evidence-console-merged2/report.json`; running-to-passed, failed-read recovery, focus/dialog, exact action, and cleanup assertions |
| Each run truthfully shows visual evidence as pending, available with its image count, invalid, unavailable, or not produced; only states with a real explanatory destination are actionable | PASS | `test.list` evidence-summary contract; nonvisual running/terminal and eight-image fixtures; no enabled empty Evidence control in the rendered interaction inventory |
| Formal verifiers with explicit caller-owned outputs also publish unique immutable governed bundles, and multiple batches in one check remain discoverable without following links or losing the legacy root bundle | PASS | formal verifier complete self-test with two custom-output publications; 8/8 focused evidence service tests covering root/nested bundles, exact image reads, symlink/unrecognized-child rejection, tamper, expiry, feedback, and list summary; complete 29/29 skill/policy graph in `.devcoordinator/agent-validation/skills-20260903T214843Z-2583011-ac9db7/check-report.json` |
| The Tests destination keeps every fact and action directly visible on phone and desktop without horizontal scrolling, clipping, overlap, or warnings | PASS | source-current formal run `formal-web-ui-mtm2hz0l-be381372`: 2/2 cells at emulated iPhone 13 and 1440×900, zero critical findings and zero warnings; both viewport/full-page image pairs reviewed and two passes finalized in `/tmp/dc2-tests-formal-merged/manual-review.json` |
| The frozen implementation preserves the execution plane, Python service, shared skills, and every Console route | PASS | 109 Rust tests; 364 Python tests passed with 28 environment skips; Ruff and JavaScript syntax clean; release executor built; complete Console and 29/29 skill/policy graphs passed |

## Infinite scrolling and highlighted test-log viewer addendum (2026-09-03)

| Item | Result | Evidence |
|---|---|---|
| Retained output has no manual paging control: the newest bounded page opens immediately, reaching the top loads one earlier cursor, the exact visible row stays anchored, short pages fill only until scrollable, and repeated scrolling reaches the truthful start boundary | PASS | focused long/short/two-cursor browser fixtures; exact row-geometry assertions; no `#test-log-page`, paging copy, or `test.log.range` call; 326/326 merged Tests interactions in `/tmp/dc2-infinite-logs-merged-focused/report.json` |
| Search and likely-failure results continue at their lower boundary; a continuation failure preserves visible output and one exact retry resumes the same operation | PASS | multi-page search/failure and failed-page/retry fixtures in the focused browser inventory; duplicate-request and programmatic-scroll guards asserted |
| Raw numbers and useful syntax/status tokens are highlighted; valid JSON and JSON-lines are indented and labelled; hostile markup remains inert escaped text with original source-line ranges | PASS | number/key/string/boolean/timestamp/failure/warning/success token assertions; whole-JSON and JSON-lines fixtures; literal script/image/event-attribute checks; no generated executable element |
| The continuous reader remains readable across mobile, the exact responsive transition, and the owner-reported viewport | PASS | `design-qa.md` final result passed; formal run `formal-web-ui-mtm3f78l-c39ddd7e` checked 15/15 raw/JSON-lines/whole-JSON cells at 390, 719/720/721, and 1095 px with zero critical findings; all thirty images reviewed and fifteen passes finalized in `/tmp/formal-web-ui-verification-TVedif/manual-review.json` |
| The frozen implementation preserves live Tests refresh, every Console route, the execution plane, and all source-owned skills | PASS | complete Console matrix 1,672/1,672 in `/tmp/dc2-infinite-logs-console-final-merged/report.json`; 109 Rust tests; 365 Python tests passed with 28 environment skips; edge 3/3; Ruff, Rust formatting, JavaScript syntax, instance-data and public-artifact checks clean; release executor built; final merged 29/29 graph recorded below |

## Grouped workers and available deployment usage addendum (2026-09-04)

| Item | Result | Evidence |
|---|---|---|
| Worker rows have no individual expanders; one counted Workers disclosure hides or restores the repository's entire worker collection while repository collapse stays independent | PASS | 333/333 merged focused interactions in `/var/tmp/dc2-workers-usage.gpYU0z/merged-focused/report.json`; pointer, Enter, Space, sibling isolation, same-session rerender, complete restore, and zero row-toggle assertions |
| A project with measurements behind an uninitialized Usage mapping shows a truthful loading state and resolves once in place without losing focus; genuine no-measurement, source-failure, and access states do not trigger detail reads | PASS | mapping-pending, unobserved, unavailable, and denied dashboard fixtures; exact `usage.repository` selector/range assertion; unchanged card link identity and focus; complete Console matrix below |
| Expanded and Workers-collapsed states remain readable at mobile, the exact responsive boundary, and the owner-reported viewport | PASS | `design-qa.md` final result passed; formal run `formal-web-ui-mtmq0rvy-d2740e86` checked 10/10 cells at 390, 619/620/621, and 1526 px with zero critical findings; five expected below-fold visibility warnings reviewed; all twenty images finalized as ten passes in `/var/tmp/dc2-workers-usage.gpYU0z/formal-web-ui-verification-uiZPGJ/manual-review.json` |
| The frozen implementation preserves every Console route and existing deployment/usage contract | PASS | complete merged Console matrix 1,679/1,679 in `/var/tmp/dc2-workers-usage.gpYU0z/merged-console-final/report.json`; 109 Rust tests; 365 Python tests passed with 28 environment skips; edge 3/3; Ruff, Rust formatting, JavaScript syntax, instance-data and public-artifact checks clean; release executor built; complete skill/policy graph 29/29 |
