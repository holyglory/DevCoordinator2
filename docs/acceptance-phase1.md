...
| Item | Result | Evidence |
|---|---|---|
| Every ready independent check starts concurrently without a host or repository budget; completion-only and success-required dependencies remain distinct | PASS | deterministic two-check barrier and graph must-catches in `tests/test_check_runner.py`; real systemd barrier, ordinary-failure continuation, completion successor, and not-meaningful dependent in `test_governed_graph_runs_all_ready_checks_and_collects_safe_failures` |
| Process exit or an exact run/check-bound event is the only normal completion signal; a long-lived setup remains available to its dependents and is cleaned afterward | PASS | unit wrong-identity/missing-event/service-exit coverage; real `test_event_completed_setup_stays_alive_for_dependent_check`; PostgreSQL follows two exact startup log events and performs one final TCP verification |
| Each check has isolated scratch, bounded output and timings; declared regular-file artifacts are content-hashed, setup runs once, and diagnostic selection/retry never substitutes for complete proof | PASS | source/artifact/symlink/privacy/cap must-catches; real per-check output, matching build reuse, stale-source refusal, selection closure, failed-check retry, and final complete rerun in `test_selection_and_failed_check_retry_remain_diagnostic` |
...
| Test and browser verification code does not use elapsed time as completion evidence | PASS | `scripts/check_no_test_timer_waits.py` scans the governed runner, lifecycle, PostgreSQL, cgroup cleanup, metrics sampler, installer, every root integration module, and Console verifier; no wait exceeds the explicit 100 ms observation fallback and the Console contains no timer wait |
| Complete current-source acceptance | PASS | 272 unit tests; all 25 root systemd/Docker/PostgreSQL/Compose/access/health integrations; `console/verify.mjs` 1,409 checks with 0 failures; Ruff; edge 3/3; instance-data scan; JavaScript syntax; diff check |
