...
mod tests {
    ...
    #[tokio::test]
    async fn recovery_preserves_an_intentional_installer_socket_fence() {
        let temporary = tempfile::tempdir().unwrap();
        let path = temporary.path().join("daemon.sock");
        let fence = temporary.path().join("daemon.pre-cutover.sock");
        let mut endpoint = SocketEndpoint::bind(&path).await.unwrap();
        fs::rename(&path, &fence).unwrap();
        assert!(!endpoint.recover_missing().unwrap());
        assert!(!path.exists());
        assert!(UnixStream::connect(&fence).await.is_ok());
        fs::rename(&fence, &path).unwrap();
        assert!(!endpoint.recover_missing().unwrap());
        assert!(UnixStream::connect(&path).await.is_ok());
    ...
    #[tokio::test]
    async fn an_unrelated_fence_does_not_prevent_missing_socket_recovery() {
        let temporary = tempfile::tempdir().unwrap();
        let path = temporary.path().join("daemon.sock");
        let fence = temporary.path().join("daemon.pre-cutover.sock");
        let mut endpoint = SocketEndpoint::bind(&path).await.unwrap();
        let foreign = UnixListener::bind(&fence).unwrap();
        fs::remove_file(&path).unwrap();
        assert!(endpoint.recover_missing().unwrap());
        assert!(UnixStream::connect(&fence).await.is_ok());
        assert!(UnixStream::connect(&path).await.is_ok());
