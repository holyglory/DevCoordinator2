CREATE TABLE work_bindings (
    ...
    native_project_id TEXT NOT NULL CHECK (length(native_project_id) BETWEEN 1 AND 128),
    workstream_id TEXT CHECK (workstream_id IS NULL OR length(workstream_id) BETWEEN 1 AND 64),
    outcome_id TEXT CHECK (outcome_id IS NULL OR length(outcome_id) BETWEEN 1 AND 128),
    experiment_ref TEXT CHECK (experiment_ref IS NULL OR length(experiment_ref) BETWEEN 3 AND 139),
