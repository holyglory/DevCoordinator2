...
const POLICY_CONTRACTS: &[(&str, usize, &[&str])] = &[
    ...
    (
        ...
        &[
            ...
            "execution failures and suggestions require diagnosis before becoming ledger outcomes",
            "create or reopen a task only when evidence establishes",
            "reuse an existing task only when its concrete outcome and acceptance scope match",
            ...
            "keep every agreed gap open until verified or explicitly removed from scope by the user",
            "knowingly leaving in-scope functionality temporarily unimplemented establishes an unfinished outcome",
            "append-only history",
    ...
    (
        "regular preliminary delivery contract",
        5,
        &[
            "prepare meaningful, runnable increments",
            "authorized non-production surface",
            "application build, executable",
            "without waiting for the whole feature or broad release validation",
            "batch small changes instead of requiring an expensive deployment after every implementation batch",
            "delivery timing follows elapsed time, not batch size",
            "keep the current preview available",
            "do not delay a requested or completed result just to wait for an alarm",
    ...
    (
        ...
        &[
            ...
            "start or attach to one delivery request for that target and deadline revision",
            "unchanged wakes do not create duplicates",
            "the first alarm never blocks ordinary implementation",
            "block the affected scope from starting a new implementation batch",
            "bounded work item already in progress",
            "finish it, and deliver immediately afterward",
            "same result, even when those commands were not already running",
            "cannot expand the current item",
            "it adds no third deadline",
            "does not reset the clock",
            "does not clear the overdue state",
            "allow only delivery recovery, necessary diagnosis and repairs",
            "independent scopes continue",
        ],
    ),
    (
    ...
    (
        ...
        &[
            ...
            "finish a passing evidence review over the agreed screens",
    ...
    (
        ...
        &[
            ...
            "functional defect",
            "before every new shipped product ui element or visual asset",
            "exactly three independent visual options",
            "actual display-order binding",
            "retained coordinator evidence",
            "user selection or explicit autonomous-selection",
            "repairs that introduce no visible element or new visual decision",
        ],
    ),
    (
